...
function playGame(userChoice) {
    ...
    if (userChoice === computerChoice) {
        ...
        result = `<span class="win-message">¡Ganaste! ${userChoice} vence a ${computerChoice}.</span>`;
    } else {
    ...
    document.getElementById('result').innerHTML = `
        <div>
            <span>Tu elección: </span><img src="${choiceImages[userChoice]}" alt="${userChoice}" class="result-img">
            <span>Computadora: </span><img src="${choiceImages[computerChoice]}" alt="${computerChoice}" class="result-img">
        </div>
        <div>${result}</div>
    `;
}
...
const moneyValues = [2000, 4000, 8000, 16000, 32000, 64000, 128000, 256000, 512000, 1024000];
...
function loadQuestion() {
    ...
    document.getElementById('money').innerText = `Dinero acumulado: $${currentMoney}`;

    const shuffledAnswers = shuffle([...questionData.answers]);
    const correctIndex = shuffledAnswers.indexOf(questionData.answers[questionData.correct]);

    questionContainer.innerHTML = `
        <h2>${questionData.question}</h2>
        ${shuffledAnswers.map((answer, index) => `
            <button class="btn answer-btn" onclick="checkAnswer(${index}, ${correctIndex})">${answer}</button>
        `).join('')}
    `;
}
...
function checkAnswer(selectedIndex, correctIndex) {
    if (selectedIndex === correctIndex) {
        ...
        if (currentQuestionIndex < questions.length) {
            ...
            alert("¡Felicidades! Has completado el juego y has ganado $" + currentMoney);
        }
    } else {
        alert("Incorrecto. La respuesta correcta era: " + questions[currentQuestionIndex].answers[questions[currentQuestionIndex].correct]);
        alert("Juego terminado. Has acumulado $" + currentMoney);
        currentMoney = 0;
